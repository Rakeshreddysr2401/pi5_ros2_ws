"""Swiggy Food MCP tools — loaded once at import, disabled gracefully.

No SWIGGY_ACCESS_TOKEN → skip the network attempt entirely (the server
requires auth; probing it just logs a 401 on every boot). A present-but-stale
token still degrades cleanly: SWIGGY_FOOD_TOOLS stays empty and the swiggy
agent tells the user food ordering isn't configured (see agents/swiggy.py).
"""
...
def _load_sync() -> list:
    if not _ACCESS_TOKEN:
        logger.info("SWIGGY_ACCESS_TOKEN not set — food ordering disabled")
        return []
    try:
        try:
