...
@tool("handover")
def handover(
    next_agent: Literal["supervisor", "chat", "local_agent", "navigate", "status", "swiggy", "tracker"],
    reason: str = "",
    chain: bool = False,
) -> str:
    ...
    Args:
        next_agent: Agent to route to — supervisor | chat | local_agent | navigate | status | swiggy | tracker
